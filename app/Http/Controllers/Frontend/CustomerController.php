...
class CustomerController extends Controller
{
    ...
    {
        $this->middleware('customer', ['except' => ['register', 'store', 'verify', 'resendotp', 'account_verify', 'login', 'signin', 'logout', 'checkout', 'forgot_password', 'forgot_verify', 'forgot_reset', 'forgot_store', 'forgot_resend', 'order_save', 'landingpageorder_save', 'order_success', 'order_track', 'order_track_result']]);
    }
    ...
    public function checkout()
    {
        ...
        $cities = District::select('district')->distinct()->pluck('district');
    ...
    public function order_save(Request $request)
    {
        ...
        $order->paid_partial_payment_amount  = $request->paid_partial_payment_amount;
        ...
        foreach (Cart::instance('shopping')->content() as $cart) {
            $order_size = $cart->options->product_size ? $cart->options->product_size : $request->product_size;
            ...
            $order_details->product_color   =   $cart->options->product_color ? $cart->options->product_color : $request->product_color;
            $order_details->product_size    =   $cart->options->product_size ? $cart->options->product_size : $request->product_size;
            $order_details->sale_price      =   $cart->price;
            $order_details->qty             =   $cart->qty;
            $order_details->save();

            if(Product::find($cart->id)->stock == 1 ){
                Productsize::where('size', $order_size)->decrement('stock', $cart->qty);
            }else{
        ...
        if (isset($password)) {
            $note = "Your account password: $password";
        } else {
            $note = '';
        }
        ...
        if ($request->payment_method == 'bkash') {
            ...
            if($request->paid_partial_payment_amount){
                $payment_amount = $request->paid_partial_payment_amount;
            }else{
                $payment_amount = $order->amount;
            }
            return redirect()->route('bkash-create-payment', ['order_id' => $order->id, 'amount' => $payment_amount, 'invoice_id' => $order->invoice_id]);
    ...
    public function landingpageorder_save(Request $request)
    {
        $this->validate($request, [
            'name' => 'required',
            'phone' => 'required',
        ...
        $shipping_area  = ShippingCharge::where('amount', $shippingfee)->first();
        ...
        $order                   = new Order();
        $order->invoice_id       = 0;
        $order->amount           = ($subtotal + $shippingfee) ;
        $order->payment_due_amount= ($subtotal + $shippingfee) ;
        $order->discount         = 0;
        $order->shipping_charge  = $shippingfee;
        $order->customer_id      = $customer_id;
        $order->landing_order    = 'yes';
        $order->order_status     = 1;
        $order->save();
        ...
        // foreach (Cart::instance('shopping')->content() as $cart) {
            ...
            $order_details->product_color   =   Color::find($request->color)->colorName;
            $order_details->product_size    =   $request->package;
            $order_details->sale_price      =   $request->product_price;
            $order_details->regular_price   =   Productsize::where('product_id',$request->product_id)->where('size',$request->package)->first()->RegularPrice;
            $order_details->qty             =   $request->quantity;
            $order_details->product_color_image = Productimage::where('product_id',$request->product_id)->first()->image;
            $order_details->save();
            // Product::where('id', $product->id)->decrement('stock', $request->quantity);
            Productsize::where('product_id', $product->id)
                        ->where('size', $request->package)
