                            ...
                            <tbody>
                                @foreach ($show_data as $key => $value)
                                    <tr>
                                        ...
                                        <td>{{ Str::limit($value->feedback, 50) }}</td>
                                        <td>{{ $value->ratting }}</td>
                                        <!--<td>-->
                                        <!--    <img src="{{ asset($value->image) }}" alt="" width="50">-->
                                        <!--</td>-->
                                        <td>
                                            <div class="button-list">
                                                @if ($value->status == 'active')
                                                    <form method="post" action="{{ route('reviews.inactive') }}"
                                                        class="d-inline">
                                                        @csrf
                                                ...
                                                @else
                                                    <form method="post" action="{{ route('reviews.active') }}"
                                                        class="d-inline">
                                                        @csrf
                                                        <input type="hidden" value="{{ $value->id }}" name="hidden_id">
                                                        <button type="button"
                                                ...
                                                <form method="post" action="{{ route('reviews.destroy') }}"
                                                    class="d-inline">
                                                    @csrf
                                                    <input type="hidden" value="{{ $value->id }}" name="hidden_id">
                                                    <button type="submit"
